...
int main() {
    ...
    while (t--) {
        ...
        if (flag) {
            cout << 0 << endl; // If unsorted, output 0
            continue;
        }

        // Find the minimum difference and corresponding pair
        int mini = INT_MAX, y, z;
        for (int i = 1; i < n; i++) {
            int diff = a[i] - a[i - 1];
            if (diff < mini) {
                mini = diff;
        ...
        int avg = (y + z) / 2; // Using integer division
        int ans = min((avg - z), (y - avg)) + 1;
...
//not yet completed , but on right track